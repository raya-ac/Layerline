...
- Static responses use kernel `sendfile` on Darwin before falling back to bounded buffered reads, can serve precompressed `.br`/`.gz` sidecars, include ETag/Last-Modified/cache headers, honor `If-None-Match` and `If-Modified-Since`, expose `Accept-Ranges`, serve single byte-range responses, and support an opt-in in-memory response cache with domain/route overrides. HTTP/2 and HTTP/3 buffered GET responses also have a conservative microcache path for eligible dynamic/proxy responses.
...
- Native HTTP/3 work is in the Zig binary: QUIC varints, HTTP/3 frame headers, QPACK literal response headers, QUIC Initial/Handshake/1-RTT packet protection, TLS 1.3 handshake flight generation, packet splitting, and buffered route dispatch for bodyless static/PHP/FastCGI/proxy responses.
- Auto Let’s Encrypt (certbot) bootstrap, ACME challenge serving from certbot webroots, periodic renewal loop, authenticated manual renewal, and systemd renewal timer assets.
...
**September 2026 audit:** this is not yet a general Caddy/nginx replacement. The [code audit and repair log](docs/audit-2026-09-05.md) records tested framing, cache, static-file, and QPACK fixes, plus the remaining HTTP/2 send-flow-control, HTTP/3 interoperability, admin-session, filesystem-containment, and memory-bound blockers. Deployment references elsewhere in this document are historical and were not reverified by this local audit.

Layerline is past the toy-server stage: the HTTP/1 path has strict parsing, header field validation, absolute-form target normalization, duplicate `Content-Length` conflict rejection, bounded bodies, keep-alive rotation, chunked request bodies, static sendfile/precompressed assets with validators and Cache-Status, an opt-in shared memory response cache with domain/route overrides, a conservative HTTP/2/HTTP/3 buffered GET microcache for eligible dynamic/proxy responses, gzip for eligible buffered responses with domain/route policy overrides, PHP CGI execution, php-fpm/FastCGI transport with worker connection pooling, PHP front-controller fallback, native HTTP/2 request-body routing, graceful GOAWAY on request caps/shutdown, route-local backend timeout/retry/health/breaker overrides, inherited global/domain/route response headers, security presets, and cache stale directives, redirects, WebSocket upgrade proxying, reverse-proxy fallback with pooled retries, configurable pool policy, least-connections, weighted, and consistent-hash balancing, reusable upstream keep-alive sockets, circuit breaker recovery, durable upstream health state, metrics, structured JSON access logs with request IDs, a local Unix admin socket with activation preflight, in-memory reload, upstream eject/recover, and certificate renewal controls, an opt-in first-launch browser admin UI with reload, upstream, certificate, and managed restart controls, named routes, host-based domain configs, direct TLS, and a companion HTTP redirect/ACME listener for owning ports 80 and 443 without Caddy. The native HTTP/3 work is in-tree and can route static, PHP/FastCGI, proxy, redirect, metrics, and health responses over QUIC/TLS 1.3 after decoding QPACK request fields; ordered and out-of-order multi-packet DATA request bodies are assembled and size/content-length checked, while WebSocket/CONNECT, flow-control depth, and broad client conformance are still on the roadmap.

The next roadmap slice is HTTP/2 send-window accounting and bounded scheduling, then the remaining security and conformance gates in the audit. Disk cache follows those correctness and resource bounds. That work builds on the existing `proxy`, `route_proxy.NAME`, `server_proxy.NAME`, and `server_route_proxy.DOMAIN.ROUTE` config surface instead of adding another parallel config style.
...
- `scripts/verify-layerline.sh` – self-starting conformance smoke for CLI doctor/certs, HTTP/1, parser hardening, HEAD error framing, h2c, h2 request bodies, request IDs, gzip, admin socket/UI reload, static validators, cache purge, best-effort external HTTP/3 curl smoke, custom 404 documents, access logs, the HTTP redirect/ACME listener, and shutdown cleanup.
...
# Local admin socket: status, validate activation config, diff activation config, redacted config inspection, cache purge, restart, routes, upstreams, cert-renew, certs, metrics.
...
# Opt-in memory cache for eligible static and buffered HTTP/2/HTTP/3 GET responses.
...
- The current native HTTP/3 path decrypts QUIC v1 Initial packets, completes a TLS 1.3 `h3` handshake with an in-process self-signed Ed25519 certificate, derives Handshake and 1-RTT packet keys, accepts a client request stream, decodes simple QPACK request fields, assembles ordered and out-of-order DATA request bodies across stream frames, and reuses the buffered route builder for static, PHP/FastCGI, proxy, redirect, metrics, health, custom 404, or built-in Layerline responses as HTTP/3 HEADERS + DATA.
...
- HTTP/3 request-body routing is intentionally still narrow: body methods are routed only when the DATA body has been assembled or Content-Length is zero; full flow-control behavior and broader client conformance remain open while HTTP/1 and HTTP/2 keep the full body-handling surface.
...
The response cache is separate from header policy. It is disabled by default, stores eligible static responses plus conservative buffered HTTP/2/HTTP/3 GET responses in process memory, and emits `Cache-Status` details for store/hit decisions:
...
The memory response cache inherits through the same global -> domain -> route chain as compression and headers. A route can turn cache off for dynamic-looking static paths without disabling it for immutable assets. The buffered microcache only stores GET responses without `Cookie` or `Authorization` and refuses responses with `Set-Cookie`, `Vary`, `private`, or `no-store`.
...
Renewal updates the certificate files on disk. `cert-renew` on the admin socket, or the browser admin certificate button, runs the configured `certbot renew` path and then reloads TLS material in memory when listener settings are unchanged. A plain manual/admin reload can also pick up renewed TLS material; the bundled systemd timer uses the same SIGHUP-compatible reload path after certbot deploys a renewed certificate.
