...
- The current native HTTP/3 path decrypts QUIC v1 Initial packets, completes a TLS 1.3 `h3` handshake with an in-process self-signed Ed25519 certificate, derives Handshake and 1-RTT packet keys, accepts a client request stream, decodes simple QPACK request fields, assembles ordered DATA request bodies across stream frames, and reuses the buffered route builder for static, PHP/FastCGI, proxy, redirect, metrics, health, custom 404, or built-in Layerline responses as HTTP/3 HEADERS + DATA.
...
- HTTP/3 request-body routing is intentionally still narrow: body methods are routed only when the ordered DATA body has been assembled or Content-Length is zero; out-of-order stream recovery and full flow-control behavior remain open while HTTP/1 and HTTP/2 keep the full body-handling surface.
