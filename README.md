...
- HTTP/3 request-body routing is intentionally still narrow: body methods are routed only when the DATA body is present in the parsed request stream or Content-Length is zero; multi-packet body assembly and full flow-control behavior remain open while HTTP/1 and HTTP/2 keep the full body-handling surface.
