...
- Multiple upstreams per route with round-robin, random, least-connections, weighted, consistent-hash, and sticky-session policies. Initial comma/space-separated upstream pools with configurable round-robin/random/least-connections selection are implemented for global, domain, and route proxy settings.
- Active and passive health checks with slow start, outlier ejection, retry budgets, and circuit breakers. Initial upstream attempt/failure/retry/ejection metrics, bounded retry budgets, and passive target cooldown are implemented; active probes and slow start remain next.
...
4. Reverse proxy upstream pools. Initial multi-target pools, round-robin/random/least-connections policies, upstream attempt/failure/retry/ejection metrics, bounded retry budgets, and passive cooldown are implemented; active health checks, weighted pools, and keep-alive upstream sockets remain next.
