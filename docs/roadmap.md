...
4. Reverse proxy upstream pools. Initial multi-target pools, round-robin/random/least-connections/weighted policies, target weights, upstream attempt/failure/retry/ejection metrics, bounded retry budgets, and passive cooldown are implemented; active health checks and keep-alive upstream sockets remain next.
