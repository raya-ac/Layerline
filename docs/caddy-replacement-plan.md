...
- Compression, cache policy, redirects, headers, health, and metrics have route/domain controls. Response header inheritance, security presets, cache shortcuts, stale directives, dynamic gzip policy, an opt-in memory cache for eligible static responses, a conservative HTTP/2/HTTP/3 buffered GET microcache, and full in-memory cache purge are implemented with route/domain-local overrides; brotli/zstd, disk cache, selective purge controls, and HTTP/1 microcache parity remain open.
