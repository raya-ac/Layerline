...
- HTTP/3 either fully routes app responses or is clearly scoped. Initial native HTTP/3 now decodes simple QPACK request fields and reuses the buffered route builder for bodyless static/PHP/FastCGI/proxy/redirect/health responses, but H3 request bodies and broad client conformance are not complete enough for a replacement claim.
