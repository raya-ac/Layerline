...
- HTTP/3 either fully routes app responses or is clearly scoped. Initial native HTTP/3 now decodes simple QPACK request fields, parses same-stream DATA request bodies, and reuses the buffered route builder for static/PHP/FastCGI/proxy/redirect/health responses, but multi-packet H3 bodies and broad client conformance are not complete enough for a replacement claim.
