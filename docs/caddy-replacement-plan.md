...
- HTTP/3 either fully routes app responses or is clearly scoped. Initial native HTTP/3 now decodes simple QPACK request fields, assembles ordered DATA request bodies across stream frames, and reuses the buffered route builder for static/PHP/FastCGI/proxy/redirect/health responses, but out-of-order stream recovery and broad client conformance are not complete enough for a replacement claim.
...
10. Admin API and web UI: validate, diff, reload, routes, metrics, upstream health/eject/recover, cert status/renewal, redacted config, and authenticated browser controls. Initial Unix socket commands plus first-launch browser setup/login/dashboard, active site inventory, live upstream controls, certificate renewal, staged main-setting writes with backups, redacted config previews, activation diff, domain-file visibility, activation preflight, in-memory reload, managed graceful restart, runtime validation, and site-file creation are implemented; richer config inspection remains.
...
Layerline is ready for narrow controlled services where HTTP/3 out-of-order stream recovery is not required and listener-bound changes can go through the managed restart bridge. It should not be described as a full Caddy/nginx replacement while HTTP/3 full routing, full protocol conformance, automated live certificate activation, and broader mutating operational admin controls are incomplete.
