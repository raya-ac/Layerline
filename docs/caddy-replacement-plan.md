...
- Reload can validate and swap config without dropping existing connections. The admin socket now exposes status, activation config preflight, runtime validation, in-memory reload, SIGHUP reload, graceful restart, routes, certs, and metrics, and the opt-in browser admin UI has first-launch setup, staged main-setting writes with backups, site-file creation, redacted config previews, enabled-file visibility, activation validation, reload, managed restart, routes, certs, and metrics. Listener-bound changes still require a managed restart.
