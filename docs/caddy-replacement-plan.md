...
- Compression, cache policy, redirects, headers, health, and metrics have route/domain controls. Response header inheritance, cache shortcuts, stale directives, dynamic gzip policy, and an opt-in memory cache for eligible static responses are implemented; brotli/zstd, disk cache, dynamic microcache, and route-local cache knobs remain open.
...
9. Cache policy: route/domain `Cache-Control`, immutable assets, stale-if-error, cache-status headers, and a bounded memory cache before a disk cache. Initial inherited `cache_control`, stale directive shortcuts, and response-cache controls are implemented, and static responses emit Cache-Status store/hit detail.
