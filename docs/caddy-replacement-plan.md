...
- Reload can validate and swap config without dropping existing connections. The admin socket now exposes status, activation config preflight, runtime validation, activation diff, redacted config inspection, in-memory reload, SIGHUP reload, graceful restart, routes, upstream health/eject/recover, certs, cert-renew, and metrics, and the opt-in browser admin UI has first-launch setup, staged main-setting writes with backups, site-file creation, live upstream controls, certificate renewal, redacted config previews, activation diff, enabled-file visibility, activation validation, reload, managed restart, routes, certs, and metrics. Listener-bound changes still require a managed restart.
...
10. Admin API and web UI: validate, diff, reload, routes, metrics, upstream health/eject/recover, cert status/renewal, redacted config, cache purge, and authenticated browser controls. Initial Unix socket commands plus first-launch browser setup/login/dashboard, active site inventory, live upstream controls, certificate renewal, staged main-setting writes with backups, redacted config previews, activation diff, domain-file visibility, activation preflight, in-memory reload, managed graceful restart, runtime validation, and site-file creation are implemented; richer config inspection remains.
