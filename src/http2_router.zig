...
pub const Callbacks = struct {
    ...
    read_static_file: *const fn (std.Io, std.mem.Allocator, []const u8, []const u8, []const u8, bool, usize, static_cache.Policy) anyerror!H2BufferedResponse,
...
pub fn buildResponseForRequest(
    ...
    if (std.mem.eql(u8, req.method, "GET") or is_head) {
        ...
        if (std.mem.eql(u8, req.path, "/") and routing_mod.domainServeStaticRoot(cfg, domain)) {
            ...
            return callbacks.read_static_file(io, allocator, routing_mod.domainStaticDir(cfg, domain), routing_mod.domainIndexFile(cfg, domain), req.headers, is_head, config_mod.maxStaticFileBytesFor(cfg, domain, null), static_cache.policyForConfig(cfg, domain, null));
        }
        ...
        if (std.mem.startsWith(u8, req.path, "/static/")) {
            ...
            return callbacks.read_static_file(io, allocator, routing_mod.domainStaticDir(cfg, domain), req.path["/static/".len..], req.headers, is_head, config_mod.maxStaticFileBytesFor(cfg, domain, null), static_cache.policyForConfig(cfg, domain, null));
        }
            ...
            !std.mem.eql(u8, req.path, "/"))
        {
            ...
            callbacks.access_log_set_handler("static_root");
            const response = try callbacks.read_static_file(io, allocator, routing_mod.domainStaticDir(cfg, domain), rel, req.headers, is_head, config_mod.maxStaticFileBytesFor(cfg, domain, null), static_cache.policyForConfig(cfg, domain, null));
...
fn buildRouteResponse(
    ...
    switch (route.handler) {
        .static => {
            ...
            return callbacks.read_static_file(io, allocator, route.static_dir orelse routing_mod.domainStaticDir(cfg, domain), rel, req.headers, is_head, config_mod.maxStaticFileBytesFor(cfg, domain, route), static_cache.policyForConfig(cfg, domain, route));
        },
        .proxy => {
